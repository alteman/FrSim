...
__attribute__((weak)) void EE_Init_Vtab() {
	;
}
...
static HAL_StatusTypeDef FLASH_ProgramHalfWord(uint32_t addr, uint16_t data) {
	return HAL_FLASH_Program(FLASH_TYPEPROGRAM_HALFWORD, addr, data);
}

static HAL_StatusTypeDef FLASH_ErasePage(uint32_t pageAddr)
{
	uint32_t pageErr;
	...
	return HAL_FLASHEx_Erase(&eraseInit, &pageErr);
}
...
uint16_t EE_Init(void) {
	...
	int16_t x = -1;
	HAL_StatusTypeDef FlashStatus = FLASH_COMPLETE;
	EE_Init_Vtab();
	/* Get Page0 status */
